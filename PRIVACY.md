...
iCloudの保存領域を画面へ公開する前に、同じprivate databaseからリセット履歴を読み取り、サーバーで
観測した世代と同じか新しい履歴が端末へ届くまで待機します。確認には期限があり、完了できなければ
記録の追加画面を開かず再試行を案内します。これは全記録の同期完了を保証するものではありません。
この読み取りのために新しい同期データ、外部送信先、運営者向けログ収集は追加しません。
