...
<https://tsumiben.hinoshiba.com/privacy/>です。
...
製品サイト、プライバシーポリシー、サポート案内はGitHub Pagesで配信します。サイト自身は
cookie、広告、解析script、行動追跡を使用しません。通常のWeb配信に伴い、GitHubはIP address、
User-Agent、access日時などの接続情報をsecurityとservice運用のために処理する場合があります。
この処理にはGitHubのプライバシーステートメントが適用され、アプリの集中記録とは結び付けません。
...
Webのサポートページからメールを送る場合、送信元メールアドレス、本文、および利用者が
任意で記載した端末機種、OS・アプリのバージョン、不具合の状況を受け取ります。これらは
返信、原因調査、購入復元の案内、セキュリティ対応のためだけに使い、広告、追跡、利用者の
集中記録との照合には使いません。メールは送信者側と運営者側のメールサービス事業者により
処理されます。機微な集中テーマ、顧客名、案件名、データ書き出しファイルを送る必要はありません。
