...
struct WrappedView: View {
    ...
    var body: some View {
        ZStack {
            ...
            RadialGradient(colors: [TsumibenTheme.amber.opacity(0.12), .clear], center: .top, startRadius: 0, endRadius: 500).ignoresSafeArea()
            ScrollView {
                VStack(spacing: 22) {
                HStack {
    ...
    var body: some View {
        ZStack(alignment: .bottom) {
            RoundedRectangle(cornerRadius: 36, style: .continuous)
                ...
                .overlay { RoundedRectangle(cornerRadius: 36, style: .continuous).stroke(TsumibenTheme.glassEdge, lineWidth: 2) }
            if revealed {
                VStack(spacing: 10) {
                    Text("ひと月のまとまり")
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack {
                ...
                Circle()
                    .stroke(TsumibenTheme.amber.opacity(0.28), lineWidth: 1)
                    .padding(8)
                Text("×\(sessions.count)")
