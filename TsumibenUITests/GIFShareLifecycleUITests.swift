...
final class GIFShareLifecycleUITests: XCTestCase {
    ...
    private func attachShareStateDiagnostics(primaryAction: XCUIElement) {
        ...
        let firstChip = app.buttons.matching(
            NSPredicate(format: "label CONTAINS %@", "#つみべん")
        ).firstMatch
        ...
        #つみべん selected=\(firstChip.isSelected) enabled=\(firstChip.isEnabled) hittable=\(firstChip.isHittable) frame=\(firstChip.frame)
