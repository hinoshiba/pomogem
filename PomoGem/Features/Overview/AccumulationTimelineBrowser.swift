...
struct AccumulationTimelineBrowser: View {
                    ...
                    ForEach(years) { year in
                        ...
                        Button {
                            ...
                            Text(year.title)
                                ...
                                .foregroundStyle(isSelected ? PomoGemTheme.background : PomoGemTheme.text)
                                .padding(.horizontal, 16)
                                .frame(minHeight: 44)
                                .background(
                                    isSelected ? PomoGemTheme.amber : PomoGemTheme.card,
                                    in: Capsule()
                                )
                                .overlay {
                                    Capsule().stroke(
                                        isSelected
                                            ? PomoGemTheme.amber
                                            : PomoGemTheme.glassEdge.opacity(0.18),
                                        lineWidth: 1
                                    )
                                }
                        }
                        .buttonStyle(PomoGemRowButtonStyle(cornerRadius: 22))
                        .accessibilityIdentifier(
        ...
        Button {
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 18)
                    .stroke(PomoGemTheme.glassEdge.opacity(0.14), lineWidth: 1)
            }
    ...
    private func timelineMetric(title: String, value: String) -> some View {
        ...
        .padding(13)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(PomoGemTheme.raised.opacity(0.72), in: RoundedRectangle(cornerRadius: 14))
    }
    ...
    var body: some View {
        Canvas { context, size in
            ...
            let bottle = RoundedRectangle(cornerRadius: 38, style: .continuous)
                .path(in: bodyRect)
            context.fill(neck, with: .color(PomoGemTheme.raised.opacity(0.30)))
            context.fill(bottle, with: .color(PomoGemTheme.raised.opacity(0.34)))
            context.stroke(neck, with: .color(PomoGemTheme.glassEdge.opacity(0.48)), lineWidth: 1.5)
            context.stroke(bottle, with: .color(PomoGemTheme.glassEdge.opacity(0.58)), lineWidth: 1.8)
