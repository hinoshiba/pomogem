...
enum HomeAtmosphere: String, CaseIterable, Identifiable {
    ...
    var ambientAccent: Color {
        switch self {
        case .midnight: PomoGemTheme.auroraBlue
        case .aurora: PomoGemTheme.auroraViolet
        case .dawn: PomoGemTheme.auroraWarm
...
struct HomeAtmosphereBackground: View {
    ...
    var accent: Color = PomoGemTheme.auroraBlue
    ...
    private var atmosphereArtwork: some View {
        ...
        case .midnight:
            ...
            RadialGradient(
                colors: [PomoGemTheme.auroraViolet.opacity(0.16), .clear],
        ...
        ZStack {
            ...
            LinearGradient(
                colors: [
                    ...
                    PomoGemTheme.background.opacity(0.16),
            ...
            if immersive, !reduceTransparency {
                RadialGradient(
                    colors: [PomoGemTheme.auroraViolet.opacity(0.13), .clear],
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for index in 0..<22 {
                ...
                context.fill(
                    Path(ellipseIn: rect),
                    with: .color(PomoGemTheme.specular.opacity(index.isMultiple(of: 5) ? 0.20 : 0.10))
                )
            }
        }
        .accessibilityHidden(true)
...
struct PomoGemCard<Content: View>: View {
...
struct PomoGemPrimaryButtonStyle: ButtonStyle {
    var tint: Color = PomoGemTheme.amber
    var foreground: Color = PomoGemTheme.background

    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.isEnabled) private var isEnabled

    init(
        tint: Color = PomoGemTheme.amber,
        foreground: Color = PomoGemTheme.background
    ) {
        self.tint = tint
    ...
    init(tintHex: String) {
        ...
        foreground = PomoGemTheme.readableForeground(onHex: tintHex)
    }
...
struct PomoGemSecondaryButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(PomoGemTheme.raised.opacity(configuration.isPressed ? 0.72 : 1))
                    .overlay {
                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                            .stroke(
                                PomoGemTheme.glassEdge.opacity(configuration.isPressed ? 0.34 : 0.14),
...
struct PomoGemBareButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.isEnabled) private var isEnabled
...
struct PomoGemRowButtonStyle: ButtonStyle {
        ...
        configuration.label
            ...
            .background {
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .fill(
                        isSelected
                            ? PomoGemTheme.amber.opacity(0.10)
            ...
            .overlay {
                RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                    .stroke(
                        isSelected
                            ? PomoGemTheme.amber.opacity(0.50)
                            : PomoGemTheme.glassEdge.opacity(configuration.isPressed ? 0.28 : 0),
...
struct PomoGemIconButtonStyle: ButtonStyle {
    var tint: Color = PomoGemTheme.text
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                Circle()
                    .fill(PomoGemTheme.raised.opacity(configuration.isPressed ? 0.70 : 0.92))
                    .overlay {
                        Circle().stroke(PomoGemTheme.glassEdge.opacity(0.18), lineWidth: 1)
                    }
            )
...
struct PomoGemCompactButtonStyle: ButtonStyle {
    var tint: Color = PomoGemTheme.amber
    var foreground: Color = PomoGemTheme.background
            ...
            .background(
                Capsule()
                    .fill(
                        isProminent
                            ? tint
                            : PomoGemTheme.raised.opacity(configuration.isPressed ? 0.72 : 0.94)
                    )
                    .overlay {
                        Capsule().stroke(
...
private struct PomoGemSheetCloseButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.isEnabled) private var isEnabled
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(
                RoundedRectangle(cornerRadius: 12, style: .continuous)
                    .fill(PomoGemTheme.amber)
                    .overlay {
                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                            .stroke(
                                PomoGemTheme.background.opacity(
...
struct PomoGemSheetCloseButton: View {
    ...
    private var styledButton: some View {
        if #available(iOS 26.0, *) {
            ...
            closeButton
                .buttonStyle(PomoGemSheetCloseButtonStyle())
        }
    }
...
struct PomoGemDestructiveButtonStyle: ButtonStyle {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.isEnabled) private var isEnabled
...
struct PomoGemHeroButtonStyle: ButtonStyle {
    var tint: Color
...
extension View {
    func pomogemNavigationTitle(_ title: String) -> some View {
        navigationTitle(title)
            .toolbarBackground(PomoGemTheme.background.opacity(0.92), for: .navigationBar)
