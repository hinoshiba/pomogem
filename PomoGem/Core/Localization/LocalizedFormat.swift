...
///
/// Buckets are always Gregorian (`StrataMath.monthLabel`, `FairnessPolicy`), so
...
/// week settings still follow the user.
...
enum DurationText {
    ...
    private static func japanese(seconds: Int, units: Units, locale: Locale) -> String {
        ...
        var text = ""
        if hours > 0 { text += grouped(hours, locale: locale) + "時間" }
        if minutes > 0 { text += grouped(minutes, locale: locale) + "分" }
...
enum MassText {
    ...
    private static func japaneseGrams(_ grams: Int, locale: Locale) -> String {
        grouped(grams, locale: locale) + "グラム"
    }
...
enum DateText {
    ...
    /// A year bucket from its number: ja 「2026年」, en "2026". Never interpolate
    /// the Int into a localized string: that prints 「2,026年」.
    static func year(_ year: Int, locale: Locale = PomoGemLocale.current, timeZone: TimeZone = .current) -> String {
        let calendar = PomoGemCalendar.gregorian(timeZone: timeZone)
        ...
        return date.formatted(style(locale: locale, timeZone: timeZone).year())
    }
...
enum SentenceText {
    ...
    static func join(_ sentences: [String], locale: Locale = PomoGemLocale.current) -> String {
        sentences.joined(separator: usesJapaneseUnits(locale) ? "" : " ")
    }
}
...
/// Whether Japanese composition applies. In the app the formatting locale
/// always follows the language the strings resolve in.
private func usesJapaneseUnits(_ locale: Locale) -> Bool {
    locale.language.languageCode == .japanese
}

private func grouped(_ value: Int, locale: Locale) -> String {
