...
iCloudを選んだApple Accountに以前のポモジェムの記録がある場合（再インストールや2台目）は、この案内の代わりに
「iCloudから記録を復元しています」を表示し、届いたテーマの件数を示して、記録が届きはじめると自動でHomeへ
進みます。待たずに進む場合は「新しく始める」を選び、確認で「新しく始める」を選ぶと上の案内へ進みます。
...
- Screen Time（1.1.0候補）: Settings → スクリーンタイム →「アクセスを許可」でindividual authorizationを
  ...
  10分ごとに、復帰後の瓶へ600秒・100gぶんの粒を追加します。無料は勉強アプリ5つまで、Proは無制限。
  「黒い石」側のアプリは無料でも無制限で、別に合計10分ごとに黒い障害物を追加します。黒同士だけで
  結合し、学習時間・共有画像には含めません。二つの集合は重複不可、カテゴリ／Webは選択不可です。
  アプリの選択と黒い石はApp Groupの端末内台帳だけに保存し、tokenをCloudKitやJSON exportへ渡しません。
...
- Screen Time 集中中のアプリ制限（1.1.0候補、任意・無料、既定オフ）: 上と同じ画面でアクセスを許可し、
  「黒い石」の「アプリを選ぶ」で控えたいアプリを1つ以上選び、「集中中のアプリ制限」の
  「集中中は気が散るアプリを開けないようにする」をオンにして「保存」。Homeで集中を開始し、ホーム画面から
  選んだアプリを開くと、iOS標準のScreen Time制限画面（ManagedSettingsのshield。独自のShield画面や
  Shield拡張はありません）が表示されます。集中画面の「今日はここまで」で中断するか、集中が終わって休憩に
  入ると解除します。一時停止中は予定の終了時刻の1分後まで続きます。設定をオフにして保存しても解除します。
  集中画面は全画面表示のため、集中中は設定画面を開けません。集中画面の外で制限が残っている場合は
  Settings → スクリーンタイムに「今すぐ制限を解除」を表示し、その集中のあいだだけすぐに解除できます。アプリが終了していても、予定の終了時刻に始まるDevice Activityの
  区間でMonitor拡張が解除を試み、次にアプリを開いたときにも解除します。対象は本人が選んだアプリだけで、
  50個を超えると制限せずに理由を表示します。本人が自分の端末で使う自己管理の機能で、他者を管理しません
  （individual authorization）。制限中は控えたいアプリを使えないため、黒い石は増えません。購入は不要です。
...
- Local projection: 瓶とまとまり粒に使う`AggregatePebble`、`Stratum`、`Bedrock`、`GachaState`は
  ...
  表示し、それ以外は「再集計中」と表示して、完了後に更新（その間の値は保存・共有しない）
- Notifications: timer終了と、任意の「集中に戻るお知らせ」。集中・休憩timer終了の通知だけを
  Time Sensitive（即時通知）として送り、本人が開始したtimerの終了をFocus中にも届ける。毎日の
  リマインダー、先月の瓶のお知らせ、集中に戻るお知らせは通常の`.active`。後者はSettings → 集中でオンにして
  通知を許可すると、計測中にアプリを離れて30秒後に一度通知し、復帰・一時停止・中断時に取り消す。
  画面ロックだけのときは、パスコードで保護されたiPhoneがロック後に送る`protectedDataWillBecomeUnavailable`を
  20秒以内に受けたら取り消す（パスコードなし等では届くことがある）。休憩中と終了間際は予約しない。
  毎日のリマインダーは、その日に集中を始めたか時間を手動で積んだ日は予約せず（Screen Timeの粒では止めない。
  前日の集中を翌朝に保存しても翌日は止めない）、最後にアプリを開いてから7日分だけ予約する。
  先月の瓶のお知らせは前の月に記録がなければ送らない。時刻は旅行先の現地時刻に従う。
  同期される通知設定を端末の通知許可で書き換えず、許可のない端末では予約しない。
...
os.Loggerへtoken、アプリ名、run ID、到達段数、黒い石の数は出さず、自動の外部送信もしません。
学習側の取り込み済み10分記録は計測済みの学習時間・質量・共有に含みます。黒い石は含めません。
