...
（[監査記録](../Docs/RealDeviceScreenTimeAudit.md)の#10）。#10はその後このブランチで修正しましたが、
**修正後のbuildで実機の利用窓を回してgemが付くことは未確認**です。
端末の「アプリとWebサイトのアクティビティ」はOFFでしたが、閾値はそれでも届いたため**原因ではありません**。
端末ログは最後まで読めておらず、件数は計測の写しから読み出しました。
**判定そのものは1つも変わりません**（不合格は不合格のままです）。
...
- [ ] 署名済みiPhoneでindividual authorizationの許可・拒否・取消・再許可と、監視エラーからの再試行を確認
  - 実機確認済み: 許可済み状態での表示整合（「アクセス許可済み」のとき「アクセスを許可」が出ない）。
    ...
    認可の見え方がプロセスごとに食い違います。この食い違いが10分到達の不合格の直接の原因です（#10）。
...
- [ ] 各側で複数アプリにまたがる合計10分・20分の実到達を確認し、復帰時に差分だけが瓶へ落ちる。
  ...
  - 実機で**不合格、原因は特定済み・修正済み（修正後の実機確認は未実施）**: 保存済み・自動記録中・監視エラーなしの状態で、
    ...
    **#10の修正はこのブランチに入りました。この項目を閉じるには、その修正を入れたbuildでgemが
    実際に付くことの実機確認が要ります（まだ一度も行っていません）。**
...
- [ ] 通知重複・順序逆転・保存失敗・途中終了・再試行で二重加算せず、保存先／アカウント／resetの
  ...
  - 実機確認済み: 保存の二重押しで保存が**1回だけ**走ること、プロセス再起動2回で取り込みが重複せず
    ...
    未確認: **到達通知の重複・順序逆転・保存失敗・途中終了からの再試行**（到達通知は届いたものの
    1件も記録へ進まなかったため実機では試せていません）、保存先／アカウント切り替え境界。
    [監査記録](../Docs/RealDeviceScreenTimeAudit.md)
...
- [ ] 黒いgemは黒同士だけで結合し、学習時間・勉強質量・レア抽選・共有画像／内訳へ混ざらないことを確認
  - 未確認: 実機で黒いgemが1個も付かなかったため（#10）、結合も分離も一度も観測していません。
    [監査記録](../Docs/RealDeviceScreenTimeAudit.md)
