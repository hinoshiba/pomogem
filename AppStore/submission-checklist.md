...
実機で確認済みです。**2026-09-22の統合候補にはPR #27として取り込み済み**です。
...
- [ ] 起動画面でオフライン継続の受領書が取り消された場合に、利用者へ非破壊の出口を残すことを実機で確認
  - 実機で**不合格を再現し、修正を別ブランチで実機確認済み。2026-09-22の統合候補にはPR #27として取り込み済み**:
    ...
    **未確認**: アプリを起動していない間にアカウントが変わる場合（通知が届かないため、この保留は働きません）。
    修正の取り込みは完了していますが、この項目を閉じるには残る条件と最終候補の確認が必要です。[監査記録](../Docs/RealDeviceScreenTimeAudit.md)の#9
...
- [ ] 署名済み実機で休憩のLive Activity（notify-06）: 集中を完走して「5分休憩」を選びロックすると、
  「集中完了」の表示が「休憩中」の残り時間に置き換わる。休憩のスキップで消え、休憩中にアプリを終了して
  開き直しても残り、background中に終わると「休憩終了」、「瓶へ戻る」や次の集中の開始で消える。
  Settingsで「画面を閉じてもタイマーを表示」をOFFにすると休憩でも表示しない
- [ ] 署名済み実機で集中の入口（notify-03 / product-04）: Home／Lock Screen widgetのtap、medium widgetの
  25／45／60／90分、Siri「ポモジェムで集中を始める」「ポモジェムで25分集中する」、Spotlight、
  ショートカットapp、アクションボタン（ショートカット割り当て）から、選択中のテーマで集中が始まる。
  集中中・休憩中のtapでは二つ目を始めず、完走カードの休憩選択待ちでは開始せずtoastを出す。
  iCloudの確認画面を経る起動でも開始し、停止画面や初回設定をまたいだ後には開始しない
