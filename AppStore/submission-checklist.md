...
**この実施で新たにチェックできた項目はありません。** 10分到達で勉強gem・黒いgemのいずれも付かず、
取消・再許可は人手が必要なまま残っています。各項目の下に、実機で確認できたことと
できていないことを書き分けます。

同日夕方、原因を切り分けるための計測を足したbuildで再試験を試みましたが、**アプリが起動できなくなり
中断しました**（オフライン継続の受領書がアカウント変化で取り消され、起動画面から非破壊の出口が消える
未修正の不具合。[監査記録](../Docs/RealDeviceScreenTimeAudit.md)の#9）。利用窓は1分も消費していないため、
**この再試験で増えた証拠はなく、上の判定は1つも変わりません**。計測は端末のbinaryへ入っていますが、
端末ログを読み出す手段が無く1行も読めていません。
...
- [ ] 各側で複数アプリにまたがる合計10分・20分の実到達を確認し、復帰時に差分だけが瓶へ落ちる。
  ...
  - 実機で**不合格**: 保存済み・自動記録中・監視エラーなしの状態で、勉強側（2アプリ合計12分）は閾値超過から
    64分待っても1粒も増えず、黒いgem側（12分）も26分待って0個ぶんのままでした。到達通知が届いた形跡が
    ありません。原因がこの変更にあるのか端末・OS側の条件かは、端末ログを取得できず切り分け未了です。
    2026-09-21の再試験は起動不能で中断し、**この判定を覆す証拠も裏付ける証拠も増えていません**。
    次に決着させる最短手は、人手で設定→スクリーンタイム→アプリとWebサイトのアクティビティを開き、
    有効かどうかと日別レポートに対象アプリの利用時間が出るかを見ることです（OFFならOSが集計しておらず、
    この不合格はアプリの側の問題ではありません）。
...
- [ ] 通知重複・順序逆転・保存失敗・途中終了・再試行で二重加算せず、保存先／アカウント／resetの
  ...
  - 実機確認済み: 保存の二重押しで保存が**1回だけ**走ること、プロセス再起動2回で取り込みが重複せず
    ...
    未確認: **到達通知の重複・順序逆転・保存失敗・途中終了からの再試行**（到達通知が1件も届かなかったため
    実機では試せていません）、保存先／アカウント切り替え境界。[監査記録](../Docs/RealDeviceScreenTimeAudit.md)
- [ ] 保存直後に`screen-time.updating`の「反映中」行が出ること、その間も戻る操作ができ、保存の
  ...
  - 実機確認済み: 保存直後も**戻る操作ができる**こと、登録内容が変わる保存では「保存」が即座に無効化される
    ...
    出現も1回しか観測できていません。[監査記録](../Docs/RealDeviceScreenTimeAudit.md)
- [ ] 黒いgemは黒同士だけで結合し、学習時間・勉強質量・レア抽選・共有画像／内訳へ混ざらないことを確認
  - 未確認: 実機で黒いgemが1個も付かなかったため、結合も分離も一度も観測していません。
    [監査記録](../Docs/RealDeviceScreenTimeAudit.md)
...
- [ ] 起動画面でオフライン継続の受領書が取り消された場合に、利用者へ非破壊の出口を残すことを実機で確認
  - 実機で**不合格・未修正**: 受領書がアカウント変化で取り消されると「端末のデータでオフライン利用」が
    描画されなくなり、成功しない「もう一度試す」とデータを捨てる「iCloudから再取得」しか残りません
    （後者はアプリが提示すらしませんでした）。スクリーンタイムの欠陥ではありませんが、この監査中に
    実機で再現し、以後アプリを開けなくしています。[監査記録](../Docs/RealDeviceScreenTimeAudit.md)の#9
