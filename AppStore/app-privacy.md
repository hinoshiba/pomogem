...
- 初回は観測した世代以上のリセット履歴が端末へ届くまで期限付きで待機する。以前の確認済み端末dataと
  ...
  account変更の通知だけでは失効させず、確認できたaccount不一致などを根拠に利用許可を失効させる。オンライン利用中に約15秒以内でbackgroundから戻った場合は同じ画面を維持する。それを過ぎて通信のない状態で戻った場合は、通信が戻ると自動で確認して元の画面へ戻り、通信がないまま今すぐ使う場合だけapp終了・再起動が必要になる。
