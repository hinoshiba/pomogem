...
|---|---|---|---|
| In-App Controls | Parental Controls | No | App独自の子ども向け監視、content filter、保護者による利用制限を提供しない。Apple Account側のAsk to BuyはApp独自機能ではない。1.1.0候補の「集中中に控えたいアプリを開けなくする」設定は、利用者本人が自分の端末で任意にオンにし、いつでも解除できる自己管理の機能で、他者を管理しない（Family Controls individual authorization）。 |
