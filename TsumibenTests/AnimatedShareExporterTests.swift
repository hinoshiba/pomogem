...
struct AnimatedShareExporterTests {
    ...
    @Test func staleCleanupOnlyRemovesOwnedExpiredGIFs() throws {
        ...
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("tsumiben-cleanup-tests-\(UUID().uuidString)", isDirectory: true)
        try fileManager.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? fileManager.removeItem(at: directory) }

        let now = Date(timeIntervalSince1970: 2_000_000_000)
        let stale = directory.appendingPathComponent("tsumiben-share-\(UUID().uuidString).gif")
        let fresh = directory.appendingPathComponent("tsumiben-share-\(UUID().uuidString).gif")
        let malformed = directory.appendingPathComponent("tsumiben-share-not-a-uuid.gif")
        let wrongExtension = directory.appendingPathComponent("tsumiben-share-\(UUID().uuidString).png")
    ...
    @Test func temporaryOwnershipRequiresPrefixUUIDAndGIFExtension() {
        ...
        let owned = directory.appendingPathComponent("tsumiben-share-\(UUID().uuidString).gif")
        let malformed = directory.appendingPathComponent("tsumiben-share-anything.gif")
...
final class ShareVisualQAArtifactTests: XCTestCase {
    func testExportsFeedStoryAndAnimatedGIFArtifacts() throws {
        ...
        attach(feed, named: "Tsumiben Share 4x5 Aurora.png")
        attach(story, named: "Tsumiben Share 9x16 Aurora.png")
        ...
        gifAttachment.name = "Tsumiben Share 9x16 Aurora.gif"
