...
それ以外の任意の1〜360分、まとまり粒の月刻印、スクリーンタイムで記録する勉強アプリ数の無制限は、1回限りのアプリ内課金
...
- スクリーンタイムで選択アプリの合計10分ごとにgemを追加。勉強アプリは無料5つ・Pro無制限、控えたいアプリの黒いgemは無料でも無制限（下記の配布前確認が必要）
...
設定から勉強アプリと控えたいアプリを別々に選び、10分単位で記録します。黒いgemは瓶の障害物として
黒同士だけでまとまり、勉強時間や共有画像には含めません。アプリの選択と黒いgemは端末内だけに保存します。
端数・通知遅延・タイマーとの二重加算回避の扱いは[仕様と実機確認](Docs/ScreenTimeGems.md)を参照してください。

この機能は既存配布版への搭載を示しません。アプリ本体とDevice Activity Monitor拡張のFamily Controls
配布権限承認、App Groupを含むprovisioning、署名済みiPhoneでの到達通知検証が出荷前に必要です。
Simulatorでbuildが通ることは、この承認や実動作の確認にはなりません。
...
SettingsからのiCloud有効化・解除を開発中です。有効化ではiCloudのデータを使って端末側を
置き換え、解除ではiCloudを端末へコピーして検証し、cloud側を残します。二つのデータを自動で
結合しません。通常のDebug／Releaseでは、複数端末間の競合を防ぎきれないため、端末データによる
iCloudの置き換えと、受付済み・別端末の置き換えの復旧再開を一時的に禁止しています。
置き換え開始前の取消しは可能です。停止中も記録と復旧用コピーを保持します。
画面で案内する再起動の際にapp自体を削除しないでください。

この機能はアップロード済み1.0.1 (7)に含まれず、上記の既存版の制約とは区別します。
[仕様と検証範囲](Docs/StorageModeTransfer.md)に成功済みのUI・source試験と残る実機・schema・release gateを
記録しています。実験用の実機試験で通った置き換え経路を、通常アプリで利用できるとは扱いません。
複数端末に関する制約は[複数端末の安全性](Docs/MultiDeviceCloudSafety.md)を参照してください。
