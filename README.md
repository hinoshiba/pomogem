...
25分・60分のタイマー、記録、瓶、iCloud同期は基本無料で利用できます。任意の
1〜180分、まとまり粒の月刻印、シェアカードの透かし非表示は、1回限りの
アプリ内課金「つみべんPro」で解放します。

![つみべんのホーム画面](http_dists/public/app-home-current.webp)

## 主な機能

- 1分を10gとして積み上げる25分／60分の集中タイマー
- SpriteKitの瓶、質量に応じた粒、タップ位置の局所衝撃、iPhoneの傾きに連動する重力
- 勉強、資格、仕事に使えるテーマの追加・編集・並べ替え・削除
- 100点、試験合格、納品などを質量0gの「成果の石」として保存
- 10個ずつ融合する可動結晶と、40年規模でも小さな積み重ねを失わない俯瞰表示
- private CloudKitによる同じApple AccountのiPhone間・機種変更後の同期
- ローカル通知、Live Activity、ホーム／ロック画面ウィジェット
- 瓶と累計質量を静止画または短いGIFとして共有
- 端末で利用可能な全11種類の保存データを、versioned JSONとして手動で書き出し
...
CloudKitへ接続せず、専用のローカル永続ストアを使います。iCloud、通知、Live
Activity、モーション、StoreKitの最終確認は実機で行ってください。
...
CloudKit、App Group、App内課金へ接続する権限は付与されません。

| 用途 | 公式版の識別子 |
|---|---|
| App bundle | `com.hinoshiba.tsumiben` |
| Widget bundle | `com.hinoshiba.tsumiben.widgets` |
| CloudKit container | `iCloud.com.hinoshiba.tsumiben` |
| App Group | `group.com.hinoshiba.tsumiben` |
| Non-Consumable IAP | `com.hinoshiba.tsumiben.pro.lifetime` |

forkを配布する場合は`project.yml`、`Shared/IntegrationConstants.swift`、
...
| 「つみべん」「Tsumiben」の名称、ロゴ、アプリアイコン、生成背景、Store／Web向けマーケティング画像 | MIT対象外。Copyright 2026 hinoshiba. All rights reserved. |
