...
SettingsからのiCloud有効化・解除を開発中です。有効化ではiCloudとこのiPhoneのどちらのデータを
残すか明示的に選び、他方を置き換えます。解除ではiCloudを端末へコピーして検証し、cloud側を
残します。二つのデータを自動で結合しません。画面で案内する再起動の際にapp自体を削除しないでください。
cloud置換前には他の端末を終了・更新する必要があり、不完全な参照関係などを検出すると復旧が
停止する場合があります。
...
記録しています。全Runtimeの切り替えや公開が完了したという記録ではありません。
