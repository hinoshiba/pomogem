...
- iCloud側を変更しない取り込み・解除は、別端末の更新により固定済みの内容比較が一致しなくなった場合、
  保存先昇格前だけ取り消してやり直せます。元の保存先とファイルを確認し、同期用コンテナを開いていない
  新しいプロセスで受付を永続化してから取り消します。元の記録・iCloud・途中のコピーはすべて保持し、
  通常の後処理でも保持コピーを削除しません。全置き換えや昇格後の処理には適用しません。
...
以下は2026-09-12の今回の利用制限追加前の検証記録です。現在の封じ込めを含む再検証結果は別途更新します。
実データ、account識別子、端末識別子、署名情報は記載しません。
前回のSimulator単体テスト全体は1,157件中1,151件成功・明示的な実機専用6件をskip・失敗0件です。
...
| 実際のSettings部品を使うDebug専用UI fixtureの保存先切り替え8ケース・オフライン7ケース、計15ケースが同一runで成功 | 二択、未選択の削除確認、戻る・取消、1回だけの開始、解除時のcloud保持、busy表示、AX5の操作・改行。オフライン案内の占有面積、全文のスクロール、44pt以上の操作、二重再試行の防止、復旧説明の取消・明示操作・履歴不一致の案内 | 通常Settingsから永続化サービスまでの全工程、実機VoiceOverの総合評価は別途必要 |
