...
2026-09-12の最新のSimulator単体テスト（Full-05）は1,188件中1,182件成功・明示的な実機専用6件を
skip・失敗0件です。Release-analyze-08のSimulator arm64／x86_64解析、OSS-currentの確認、
スクリプト回帰12件も成功しました。実データ、account識別子、端末識別子、署名情報は記載しません。
Release構成の置き換え禁止ポリシー5件もすべて成功しました。検証コピーでテスト対象だけを限定し、
...
| 利用制限を含むDebug専用UI fixtureで保存先切り替え8ケース・オフライン7ケース、計15ケースが最新UI-07の同一runで成功 | iCloud置き換えの禁止表示と操作不能、使える選択肢と未選択の削除確認、戻る・取消、1回だけの開始、解除時のcloud保持、busy表示、AX5の操作・改行。オフライン案内の占有面積、全文のスクロール、44pt以上の操作、二重再試行の防止、復旧説明の取消・明示操作・履歴不一致の案内 | 通常Settingsから永続化サービスまでの全工程、実機VoiceOverの総合評価 |
...
通常Hostの起動でオンライン確認済みの利用記録が生成されたことを、実画面操作や通信断の合格へ広げません。
