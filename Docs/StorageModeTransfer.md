...
それぞれの確認alertに置き、確認alertだけが選択を確定します。「このiPhoneだけに保存しますか？」では、
アプリを削除すると記録が失われること、あとでiCloud同期に切り替えるとこのiPhoneの記録がiCloudの記録に
置き換わること、このiPhoneの記録をiCloudへ移すことは現在できないことを示します。
