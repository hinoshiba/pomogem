...
長い休憩を提案し、10分×10、25分×4、60分をまたぐ経路で同じ境界になります。同じ完走IDを
...
- Homeは瓶と開始を中心に、開始前のテーマ・時間を直接選べるボタンを併置。設定変更だけでは開始せず、詳細操作はメニューにまとめる。メニューは記録・積み上がり・シェア・設定を先頭に置き（半分の高さでも記録と設定が見える）、手動・成果の追加、積み上がり計画、背景（集中する空間）の順に続ける
