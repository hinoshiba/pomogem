...
| 課金 | 複数プランに見せる、偽の期限、価格の不一致、復元困難がないか。無料で使える範囲を隠していないか。本人の取り消しを失敗扱いしていないか。ご褒美の画面を広告にしていないか | 無料download＋買い切り1商品だけとし、日本JPY 100、米国USD 0.99基準、その他はAppleの現地相当額をStoreKitの`displayPrice`で提示。復元、条件、法的リンク、閉じるを常時提供。入口の機能を先頭に置き、無料の範囲（25・45・60・90分、勉強アプリ5つ）とほかの機能が無料であることを同じ画面で示す。Apple Accountの画面を閉じた取り消しには何も表示せず、失敗は原因別の平易な文と専用の見出しで伝え、フレームワークの文言は出さない。承認待ち（Ask to Buy）は24時間で消える表示用の目印で、購入ボタンを止めず、権利の判定に使わない。承認が届いたときの知らせは、タイマーやシートが閉じて画面に見えるときに1回だけ出し、ペイウォール自身の購入・復元では重ねて出さない。結晶の画面の「Proなら…」は無料の人に端末ごと一度だけ、全操作の下の小さなリンクとして出し、モーダル・バッジ・繰り返しはない。App Storeの評価は設定の行を本人が選んだときだけ開き、自動の評価依頼は従来の条件（10回の完走と最初の完走から7日）のまま |
...
- 記録の「月ごとの瓶」（直近12か月）は、年月と同じ`AccumulationTimelineRepository`で12か月を1回の有界区間として正確に集計し、月ごとの12ページを読まない。`@ModelActor`は専用スレッドを持たず、SwiftDataの既定executorは待っている側のスレッドで処理を実行する（iOS 26.5で計測。SwiftUIの`.task`から直接awaitするとメインスレッドで走る）。そのためViewは必ず`AccumulationTimelineLoader`（detached taskから呼び、キャンセルを転送する）経由で読む。記録の今週／今月のページ・最新30件とまとまり粒・月一覧、年月の範囲・年・月、日シートが対象で、`AccumulationTimelineRepositoryTests`がメインスレッド外で走ることと、View側がRepositoryを直接生成しないことを、`LogHistoryReadsTests`が記録の値がメインで読んでいたときと同じであることと、LogViewが生涯分に比例する読み込み（セッションのページ、まとまり粒）を自分で行わないことを検査する。Repositoryが返すのは値だけで、SwiftDataのobjectはactorの外へ出さない。記念石だけは記念石の表から最大61件を読むだけで、編集・削除・元に戻す直後に同じ画面で読み直すため、メインのcontextで読む。ひとつのstoreへの要求はCore Dataの1本のSQL queueで順に実行され、メインのcontextの読み込みが外の長い読み込みの後ろに並ぶとその間メインスレッドが止まるため、記念石は各読み込みがメインスレッド外の読み込みを始める前に読む
- 記録の読み込みは依存するものごとに3つに分ける。今週／今月のページ（切り替えで読むのはこれだけ）、期間に依存しない最新30件・記念石・まとまり粒、月一覧。Control Center・通知センターの開閉（inactive）ではどれも再読込せず、バックグラウンドからの復帰で読み直す（読み直している間は前の表示を残す）。最初の読み込みが終わるまでは「この期間の粒は、まだありません。」「一粒積むと、ここに記録が残ります。」などの空の文言を出さず読み込み中を示す（WrappedViewも同じ再読込規則）。今週／今月を切り替えた直後は、新しいページが届くまで前の期間の表示を、その期間の見出し・日付のまま残す（多くの端末では一瞬）。250msを超えたら読み込み中の表示に替え、別の期間の数字を新しい期間の名前で見せない。resetより前に読んだページは表示しない。期間の日付（「9月21日(日)〜9月27日(土)」）は、表示中のページを読んだ週・月のものを出す。週・月の境目をまたいでから戻ったときも、新しいページが届くまでは数字と日付が同じ期間を指す。読み込み中の数字のタイルは形だけを見せ、VoiceOverには「記録を読み込み中」とだけ伝える（「0分、積んだ時間」と読ませない）。最新30件の読み込みに失敗し、その時点でまだ何も読めていなければ、「最近の記録を読み込めませんでした。」と出す。読み込み中のままにも、「一粒積むと、ここに記録が残ります。」にもしない。まとまり粒アーカイブは集計（projection）から作るため、読んだときのiCloudの確認（`AggregateProjectionCacheStamp`）がいまも有効なときだけ表示する。確認済み→再集計中→確認済みと変わった後は、新しい確認の下で読み直したものが届くまで出さない。メインスレッド外の3つの読み込みは同時に始めず、1つずつ読む（`LogReadQueue`）。Core Dataは1つのstoreへの要求を1本のSQL queueで順に実行するため、同時に読んでも速くならず、ほかの画面がメインのcontextで読むとき（Homeのquery、バックグラウンドから戻ったときの通知の確認など）にその全部を待たせるだけになる。今週／今月のページは、待っている読み込みより先に読む。40年fixture（350,640件）では、記録を開いたとき・今週／今月を切り替えたとき・バックグラウンドから戻ったときのそれぞれで、読み込みが始まってから全部届くまでのメインスレッドの最長停止が500ms未満、記録を開いてから最初の読み込みが始まるまで（画面の切り替えと最初の描画）が1.5秒未満であることを、`FortyYearPersistentColdLaunchUITests`がアプリ内の計測（`LogLoadAudit`、DEBUGのUIテスト起動だけ）で確かめる。計測の間、テストはアプリに問い合わせず、計測の終わりをDarwin通知で待つ（XCUIの問い合わせはアプリのメインスレッドでaccessibility treeを作るため、それ自体が停止に見える）
