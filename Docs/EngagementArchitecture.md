...
- アプリの外の入口（ホーム／ロック画面ウィジェット、`pomogem://`リンク、Siri・Spotlight・ショートカット・アクションボタンの「集中を始める」）は、本人がその入口で集中を始めると選んだときだけ、ホームの開始ボタンと同じテーマ・時間・条件で集中を始める（`FocusStartEntryPolicy`）。進行中・復元待ちのタイマーがあれば二つ目を始めず、完走カードの休憩の選択やテーマの選択を飛ばさない（理由をtoastで示す）。要求は60秒で失効し、起動時の停止画面や初回設定をまたいで持ち越さない。休憩の終わりや通知から次の集中を自動で始めることはない
