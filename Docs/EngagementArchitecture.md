...
| 通知は注意を中断し、頻度や制御不能感がストレスになり得る | 通知は既定オフ、時刻を本人が選び、テーマ名は端末外の通知へ出さない。毎日のリマインダーは、その日に集中を始めたか記録を積んだ日と集中中は鳴らさず、最後にアプリを開いてから7日で休む（開けば静かに再開）。先月の瓶のお知らせは記録のない月に送らない。時刻は現地時刻に従う。端末の通知許可で同期設定を書き換えない |
