...
- 年月の月シートは、同じ正確集計からテーマ別の時間と割合、日ごとの記録（時間・粒数・テーマ色）を追加の読み込みなしで示し、日を選ぶとその日の全記録を1日の有界区間として読む日シートを開く。「この月の瓶を見る」で12か月より古い月もWrappedとカードにできる（年月はシート上にあるため、カードはWrappedの上に開く）。記録からは「過去の記録を月・日ごとに見る」と、グラフの棒（VoiceOverではカスタムアクション）から同じ日シートへたどれる。記録の最新30件は延長せず、生涯一覧のページングも行わない。同期中に変わり続けるCloudKit複製の集合を安全にページングする仕組み（SyncMaintenanceArchitecture §7.3、roadmap 3）が整うまでは、日と月の有界区間で答える。活動した日数は数えず、連続日数のような表示にしない
...
- 記録の「月ごとの瓶」（直近12か月）は、年月と同じModelActorで12か月を1回の有界区間として正確に集計し、メインスレッドで月ごとの12ページを読まない。今週／今月の切り替えとControl Center・通知センターの開閉（inactive）では再読込せず、バックグラウンドからの復帰で読み直す。最初の読み込みが終わるまでは「この期間の粒は、まだありません。」などの空の文言を出さず読み込み中を示す（WrappedViewも同じ再読込規則）
...
3. Overviewのon-demand正確集計と同じepoch／UUID規則をLog／Share／Wrappedへ広げ、永続化summary + ページングへ発展させる（記録の月別合計はModelActorの区間集計へ移行済み。今週／今月のページと最新30件は有界のままメインスレッドで読む）
