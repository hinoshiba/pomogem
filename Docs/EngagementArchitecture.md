...
| 課金 | 複数プランに見せる、偽の期限、価格の不一致、復元困難がないか。無料で使える範囲を隠していないか。本人の取り消しを失敗扱いしていないか。ご褒美の画面を広告にしていないか | 無料download＋買い切り1商品だけとし、日本JPY 100、米国USD 0.99基準、その他はAppleの現地相当額をStoreKitの`displayPrice`で提示。復元、条件、法的リンク、閉じるを常時提供。入口の機能を先頭に置き、無料の範囲（25・45・60・90分、勉強アプリ5つ）とほかの機能が無料であることを同じ画面で示す。Apple Accountの画面を閉じた取り消しには何も表示せず、失敗は原因別の平易な文と専用の見出しで伝え、フレームワークの文言は出さない。承認待ち（Ask to Buy）は24時間で消える表示用の目印で、購入ボタンを止めず、権利の判定に使わない。結晶の画面の「Proなら…」は無料の人に端末ごと一度だけ、全操作の下の小さなリンクとして出し、モーダル・バッジ・繰り返しはない。App Storeの評価は設定の行を本人が選んだときだけ開き、自動の評価依頼は従来の条件（10回の完走と最初の完走から7日）のまま |
