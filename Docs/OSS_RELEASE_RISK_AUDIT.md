...
|---|---|---|---|
| 対処済み（2026-09-06追記） | Git履歴の公開identity | ownerの明示指定で既存13 commitのauthor／committerメールを kai.openclaw01@gmail.com へ統一。元の履歴はrepository外にbackupし、各tree・日時・messageを照合 | 指定アドレスだけを個人メール検査の例外とし、raw historyと公開candidateを再検査する。手順は`Docs/OSS_PUBLISHING.md`を参照 |
