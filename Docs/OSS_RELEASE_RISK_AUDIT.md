...
Target: Tumiben 1.0 / Apple ID `6806758060` / `com.hinoshiba.tumiben`
...
現時点の判定は **NO-GO** です。build 1のupload、host／Widgetのexplicit App ID、
hostのiCloud container割当、45／90分無料枠、Live Activity、単一テーマ一覧と開始button長押し変更を含む
build 4の開発署名Archiveと
raw Archive検証までは成立しましたが、build 4のdistribution、CloudKit production schema、
署名済み2台実機、Sandbox、TestFlight、法定表示と
販売主体判断、Web公開、rights sign-offが未完了だからです。local candidateは、7種類の同期元modelと
...
|---|---|---|---|
| Blocker | Git履歴に公開承認されていないidentity metadataが残る | 2026-09-05の`check-oss-readiness.sh`はreachable commit `ed7f63b`のauthor／committer emailがallowlist外として失敗した。`.mailmap`や後続commitではraw objectを除去できない | 公開してよいidentityかownerが明示承認してcheckerのallowlistと方針文書を更新するか、公開前にreachable historyと既存remoteを合意のうえ安全に書き換え、GitHub noreplyまたは承認済みsupport addressだけになったことをraw object scanで再確認する。履歴書換え／force pushはowner承認なしに行わない |
| Blocker | 現行build 4のCloudKit production schema／distribution payloadが未成立 | 2026-09-04にhost App IDのiCloud／CloudKit、Push、IAPとWidget App IDを確認。振った際のgem物理移動と消音中も独立して動作する触覚設定を含む最新のversion 1.0 (4)は2026-09-05にArchiveし、raw development署名、host／Widget、CloudKit／APNs development環境、build番号、embedded extension、Privacy Manifest、arm64、Release debug-gate、account-neutral Widget／Live Activity markerの検証に合格した。ownerが直前のbuild 3をDistributeした旨は確認したが、build 4のApp Store用distribution payloadは未作成 | development schemaだけをclearし、最終7-model source schemaを検証後productionへdeployする。検証済みbuild 4をDistributeし、exported payloadでhost／widgetのdistribution署名、production CloudKit／APNs、build番号、embedded extension、hostのLive Activity key、WidgetのActivityKit linkageとaccount-neutral markerを再検証する。productionはclearせず、既存certificateを推測でrevokeしない |
| Blocker | 実CloudKitでsource同期とmaintenanceが未検証 | bounded worker、kind別durable retry、split store、account namespace、Lamport reset、projection rebuild、source duplicateのread-only resolverはlocal実装／test sourceがあり、2026-09-05の全595件は594件成功、明示opt-inの40年soak 1件skip、失敗0件。Release ArchiveとAnalyzeも成功し、error／warning／analyzer warningは0件。production CloudKitの実配信順序、account切替、分断・再接続、killは未完走 | 同じApple Accountの署名済み2台で異なるoffline session、timer引き継ぎ、A/B→B/C→late Cのpartial duplicate delivery、620件以上のlocal projection再構築、resetを合格する。current／awaiting source fingerprintが不変で、foreign rowへのsave／delete／fan-outがなく、account A→Bはblock、onlineのAへ戻ると元のnamespaceを再mountしてstore／focus／checkpointを混ぜないことも確認 |
| Blocker | App Store提出物がaccountへ未保存 | ja-JP／en-US原稿、privacy／age／review draftと、production UIをDebug-only deterministic fixtureでcaptureしてdirect deletion rowがないことを確認した5枚の1284×2778 screenshotはlocalにある。signed Release実機とのvisual parityと、version 1.0のcategory、privacy、価格／提供地域、IAP、buildは未確認／未保存 | signed Release実機との差を確認して必要なら再captureし、source-of-truthどおり入力。reload後も値が保持され、buildとIAPを同一submissionへ追加 |
| Blocker | 日本向け有料IAPの法定表示と販売主体の判断が未完了 | 販売条件pageと購入前linkは実装され、2026-09-04に公開`/commercial-transactions/`のHTTPS 200を確認した。法定氏名／住所／電話の非公開正本、請求時開示の送受信、担当者不在時手順は未検証。Apple契約はIAP UIの法定開示をdeveloper責任とする | 特商法の適用と必要表示をowner／専門家が確認。該当時は購入前導線と実運用を完成し、提出直前にも公開pageがHTTPS 200であることを再確認する。個人情報は適切な管理手段で扱う。代替は1.0を無料・IAPなしで再設計 |
...
| Blocker | 実購入・同期・端末品質を未検証 | production CloudKit、StoreKit Sandbox、2台実機、build 4のTestFlight完了記録なし | checklist記載の購入／復元／失効、CloudKit、offline、account-neutral Widget／Live Activity、通知、exportを実機で合格 |
...
| High | repositoryが非公開のままならOSS配布にならない | 匿名GitHub accessは404。local GitHub CLIも未認証 | release commitのsecret／rights監査後にPublic化し、公開状態を匿名sessionで再確認 |
...
| High | CloudKit初回schema／future migrationでデータ損失・起動失敗 | 1.0はpublic buildが存在しないためReleaseは最終split topologyを直接作成し、危険なmonolithic reopen migrationはDEBUGだけ。最終RCでは`syncRecordID`をSubject／StudySession／AchievementStone／Prefs／FocusTimerDeviceClaimへ、Subject revision／tombstone、Achievementの`deletionRevision`／`deletionMutationID`／`restoredDeletionMutationID`、Prefs `settingsWriterID`、`timerCompletionSoundRawValue`／`timerCompletionHapticRawValue`と12組（24 field）のrevision／mutation stampを追加したが、production schemaは未deploy | development environmentだけをclearして最終RCから再initializeし、field名・型・default、`timerCompletionSoundRawValue`／`timerCompletionSoundRevision`／`timerCompletionSoundMutationID`、`timerCompletionHapticRawValue`／`timerCompletionHapticRevision`／`timerCompletionHapticMutationID`、legacy Achievement event=`(row.revision, deletionMutationID ?? syncRecordID)`、clean install、2台partial delivery、raw exportを検証後、そのexact 7-model schemaをproductionへpromoteする。production environmentはresetしない。将来の既存user migrationはreleased store fixtureとversioned migration planなしに追加しない |
...
| Medium | iPhone appが未試験のMac／Vision Proへ自動提供される | App Store Connectで両方のavailabilityが現在有効 | 両方を無効化し、将来有効化する場合は専用QAを実施 |
...
| Medium | support窓口やpolicy pageが止まるとreview／利用者対応が成立しない | 現在HTTPS 200だが、実メール送受信とretention手順は未検証 | URL監視、support mailbox送受信、削除／export依頼手順、障害時ownerを確認 |
| Medium | Web security header不足 | 2026-09-04再測定でHTTPはHTTPSへ301、HTTPSは200だが、HSTS／CSP／nosniff／Referrer-Policy／Permissions-Policyは未確認または不足。DNS、response header、IPはGitHub Pages配信を示し、Cloudflare利用は確認できない | 必要なsecurity headerをPagesで設定できない場合は、実在を確認した対応hosting/CDNへ移行して再測定し、その時点で実際に使うserviceだけをprivacy policyへ記載する |
| Medium | GitHub Pages custom domainの所有確認が外部から立証できない | 2026-09-03実測でGitHub Pages verification TXTなし。別の既存検証方法が有効な可能性はある | Repository／organization SettingsのPagesでOwner verifiedを確認し、未検証ならGitHub指定TXTを追加。CNAME takeover耐性を匿名確認 |
...
3. MIT対象外assetは`ASSET_LICENSES.md`のhashと一致させ、OFL fontにはlicense全文を同梱する。
   Pages deployは公式`hinoshiba/Tumiben` repositoryだけにguardし、forkがbrand asset入りWebを
   誤公開しないようにする。
...
2. automatic signingが作るportal stateは、maintainerの許可後だけ変更する。既存certificateを推測で
   revoke／再作成しない。
3. public buildがない1.0のdevelopment CloudKit schemaだけをclearし、最終RCから7 source modelを
   reinitializeする。`syncRecordID`、Subject revision／tombstone、Achievement `deletionRevision`／
   `deletionMutationID`／`restoredDeletionMutationID`、Prefs writer、`timerCompletionSoundRawValue`／
   `timerCompletionHapticRawValue`、12組（24 field）のstampを検査し、検証済みの
...
- Sandbox、2-device、TestFlight結果、development schema clear／最終RC再initialize検証記録、exact 7-model
