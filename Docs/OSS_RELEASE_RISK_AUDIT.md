...
timer表示変更前のbuild 4の開発署名Archiveと
raw Archive検証までは成立しましたが、`timerDisplayMode`を含む最終候補の再Archive／distribution、
...
| Blocker | `timerDisplayMode`を含む最終候補のCloudKit production schema／distribution payloadが未成立 | 2026-09-04にhost App IDのiCloud／CloudKit、Push、IAPとWidget App IDを確認。振った際のgem物理移動と消音中も独立して動作する触覚設定を含むversion 1.0 (4)は2026-09-05にArchiveし、raw development署名、host／Widget、CloudKit／APNs development環境、build番号、embedded extension、Privacy Manifest、arm64、Release debug-gate、account-neutral Widget／Live Activity markerの検証に合格した。ただし、このArchiveは`timerDisplayModeRawValue`と対応するstamp fieldの追加前であり、最終候補を証明しない。ownerが直前のbuild 3をDistributeした旨は確認したが、変更後候補のApp Store用distribution payloadは未作成 | development schemaだけをclearし、13 group・26 stamp fieldを含む最終7-model source schemaを検証後productionへdeployする。変更後候補を一意なbuild番号で再Archive／Distributeし、exported payloadでhost／widgetのdistribution署名、production CloudKit／APNs、build番号、embedded extension、hostのLive Activity key、WidgetのActivityKit linkageとaccount-neutral markerを再検証する。productionはclearせず、既存certificateを推測でrevokeしない |
| Blocker | 実CloudKitでsource同期とmaintenanceが未検証 | bounded worker、kind別durable retry、split store、account namespace、Lamport reset、projection rebuild、source duplicateのread-only resolverはlocal実装／test sourceがある。`timerDisplayMode`追加後候補では2026-09-05にXCTest 598件中597件成功、明示opt-inの40年soak 1件skip、Swift Testing 25件成功、失敗0件。Release Simulatorのbuild／Analyzeもerror／warning／analyzer warning 0件だった。Release Archive、production CloudKitの実配信順序、account切替、分断・再接続、killは未完走 | 一意なbuild番号の最終候補でRelease Archiveを作成する。同じApple Accountの署名済み2台で4種類のtimer表示選択、別groupとのoffline同時変更、同じgroupの競合、異なるoffline session、timer引き継ぎ、A/B→B/C→late Cのpartial duplicate delivery、620件以上のlocal projection再構築、resetを合格する。current／awaiting source fingerprintが不変で、foreign rowへのsave／delete／fan-outがなく、account A→Bはblock、onlineのAへ戻ると元のnamespaceを再mountしてstore／focus／checkpointを混ぜないことも確認 |
