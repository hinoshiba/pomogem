...
次候補の保存先切り替えはアップロード済み1.0.1 (7)とは別機能です。切り替え中の復旧画面では、
案内に従ってappを終了・再起動してください。appの削除・再インストールを復旧手順にしません。
account確認、完全なコピー、参照先の対応を検証できない場合は保存先を勝手に確定しません。
不完全なrelationshipや対応が曖昧な記録は、安全に自動復旧できず停止する場合があります。

iCloudを端末データで置き換える前に、他の端末のappを終了し最新版へ更新してください。
旧版やoffline端末の古い記録が後から届く可能性があります。解除ではcloudを端末へコピーして
cloud側を残します。通常launchで不正な保存履歴を新しいnamespaceへ逃がす処理とは異なり、
明示したdurable journalまたはcommit済みreceiptで認めた変更だけを扱います。
[仕様と試験の範囲](StorageModeTransfer.md)を参照してください。
