...
|---|---|---|
| iCloudのデータが置き換わりました | サーバに確定済みの世代があり、この端末の受領記録と合わない | 「iCloudから再取得」＝この端末の記録を捨ててiCloudの内容を取り直す。「このiPhoneのデータで置き換える」＝iCloudの内容を捨ててこの端末の記録を送る（各々に独立した確認が必要）。「先にこの端末の記録を書き出す」＝控えの保存のみ |
| iCloudの管理情報が見つかりません | サーバに転送台帳が無い（レコードが無い、または確定済みの世代が無い） | 「このiPhoneのデータでiCloudを使い始める」＝「最後の確認」を経て、この端末の記録をiCloudへ送り、新しいiCloudのデータとして使い始める。**いまiCloudに残っている記録は削除されます**。そのため先に読み取り専用でiCloudの件数を確認し、その結果が出るまでボタンは押せません。「端末のデータでオフライン利用」＝iCloudへ何も送らず、端末の記録のまま使う。「もう一度試す」＝もう一度iCloudを確認し直す（何も削除しません） |
...
「このiPhoneのデータでiCloudを使い始める」は、**現在の配布ビルドでは無効表示**です
（`StorageTransferReleasePolicy.standard.allowsDatasetOverwriteFromDevice`が`false`のため）。
画面には無効である理由も表示します。有効化はPLAN §11.1のオーナー判断と2台での検証待ちです。
...
アプリが読み取り専用でiCloud側の件数を数えます。

- iCloud側に記録がある場合は、その件数（と管理情報の有無）を確認画面に表示します。
- iCloud側が**0件**だった場合は、その旨を専用の警告として表示します。iCloudのPomoGemの
