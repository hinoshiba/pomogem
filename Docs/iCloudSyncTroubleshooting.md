...
| iCloudの管理情報が見つかりません | サーバに転送台帳が無い（レコードが無い、または確定済みの世代が無い） | 「このiPhoneのデータでiCloudを使い始める」＝「最後の確認」を経て、この端末の記録をiCloudへ送り、新しいiCloudのデータとして使い始める。「端末のデータでオフライン利用」＝iCloudへ何も送らず、端末の記録のまま使う |
| 別のiCloud環境のデータです | この端末の受領記録が、別のCloudKit環境（開発用／配布用）で作られている | 破壊的な操作は置きません。「もう一度試す」「端末のデータでオフライン利用」「サポートを見る」のみ |
| iCloudのデータを受け取った記録がありません | 端末側の台帳が欠けており、サーバ側にも確定済みの世代が見つからない | 同上。再取得の選択肢は表示できないため、その理由を説明します |
...
文面で明示します（捕まえたエラーの文面を汎用画面に載せません）。
