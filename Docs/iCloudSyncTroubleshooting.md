...
出ている場合は、先にそれを完了するか閉じてから「もう一度試す」を押してください。記録や保存先の設定は
変更していません。」と表示し、「もう一度試す」と、この端末がオフライン利用の条件を満たしていれば
「端末のデータでオフライン利用」を出します。期限切れはアカウントの確認結果ではないため、保存方式を
決めたり、記録を削除したり、オフライン利用の許可を勝手に与えたりはしません。バックグラウンドへ移った
場合は期限を解除し、OSによる再開を待ちます。
